...
{
	if (0 && _player(Event::CEILING) && _player.currentActionId() != STICK_CEILING && !_player[DOUBLE_JUMP]->val())
	{
	...
	if (_count)
		_count--;
	else if (_player(Event::CEILING) && _player.currentActionId() != STICK_CEILING)
	{
		_count = 3;
		if (_player[TECH]->val())
		{
